...
function() {
    ...
    app.Chatroom = Backbone.Model.extend({
        defaults: {
            errors: [],
            errfor: {},
            username: "游客-->88#" + Math.floor(1e5 * Math.random()),//document.getElementById("username").value,//
            message: "",
            userList: [],
            numUsers: 1
        }
    ...
    app.ChatroomView = Backbone.View.extend({ 
        ...
        initialize: function() {			
			...
			//Eric +
			this.model.set("username", document.getElementById("username").value);
        ...
        initYunbaSDK: function() {
            ...
            app.yunba_demo.init(function(t) {
                //t ? (e.logMessage("初始化成功..."), e.connect()) : (e.logMessage("初始化失败或服务断线，若长时间无响应请尝试刷新页面"), e.connect())
				t ? e.connect() : (e.logMessage("初始化失败或服务断线，若长时间无响应请尝试刷新页面"), e.connect())
            },
            function() {
        ...
        connect: function() {
            ...
            app.yunba_demo.connect_by_customid(t,
            function(t, a) {
                //t ? (e.logMessage("连接成功..."), e.setMessageCallback(), e.subscribe(app.chatroomTopic)) : e.logMessage(a)
				t ? (e.logMessage(""), e.setMessageCallback(), e.subscribe(app.chatroomTopic)) : e.logMessage(a)
            })
        },
        subscribe: function(e) {
            ...
            function(e, a) {
				var log = app.Chatroom.username + " 进入狼窝...";
				e ? (t.logMessage(""), t.sendState("ONLINE"), t.getOnlineUsers(), $(".chatroom-btn").attr("disabled", !1)) : t.logMessage(a)
                //e ? (t.logMessage("加入房间成功..."), t.sendState("ONLINE"), t.getOnlineUsers(), $(".chatroom-btn").attr("disabled", !1)) : t.logMessage(a)
            })
        },
        ...
        sendMessage: function() {
			...
			//alert(app.Chatroom.username);
            if ("" !== $("#chatroom-input").val()) {
                var e = JSON.stringify({
                    dataType: "MESSAGE",
                    dataContent: $("#chatroom-input").val(),
                    username: this.model.get("username")  // app.Chatroom.username//
                });
                this.publish(app.chatroomTopic, e),
                $("#chatroom-input").val("")
            }
        ...
        userController: function(e) {
			...
			//this.logMessage("dataType: "+e.dataType+"----, dataContent: "+e.dataContent);
			//if ("ONLINE" === e.dataContent)
			//this.logMessage(o + " 进入房间...123")
			...
			if (!isExist) {
				this.logMessage(o + " 进入狼窝...")
			}
			
            if ("ONLINE" === e.dataContent) - 1 === t.indexOf(o) && (t.push(o), a = t.length, this.addOnlineUserElement(o));
            else if ("OFFLINE" === e.dataContent) {
                var n = t.indexOf(o); - 1 != n && (t.splice(n, 1), a = t.length, this.removeOnlineUserElement(o))
            }
        ...
        removeOnlineUserElement: function(e) {
			...
			this.logMessage(e + " 离开狼窝...")
        },
        addMessageElement: function(e, t) {
            var a = $("#chat-messages");
            if (t) return a.append($("<li>").addClass("chat-log").text(e.log)),
        ...
        sendMessageOnEnter: function(e) {
            13 === e.keyCode && (e.preventDefault(), "chatroom-input" === $(e.target).attr("id") && this.sendMessage())
        },
        ...
        webNotifications: function(e, t) {
            var t = JSON.parse(t);
            if (t.username === e.model.get("username")) return 0;
            if (window.Notification) if ("MESSAGE" === t.dataType || "PICTURE" === t.dataType || "MICROEXP" === t.dataType || "VOICE" === t.dataType || "FILE" === t.dataType || "VIDEO" === t.dataType) {
                var a = navigator.userAgent.toLowerCase();
                a.indexOf("safari") != -1 && a.indexOf("chrome") > -1 ? Notification.requestPermission().then(function(e) {
                    if ("granted" == e) {
                        if ("PICTURE" === t.dataType || "MICROEXP" === t.dataType) var a = new Notification(t.username, {
                            body: "[图片消息]",
                            icon: "/static/img/user.png"
                        });
                        else if ("VOICE" === t.dataType) var a = new Notification(t.username, {
                            body: "[语音消息]",
                            icon: "/static/img/user.png"
                        });
                        else if ("FILE" === t.dataType) var a = new Notification(t.username, {
                            body: "[文件消息]",
                            icon: "/static/img/user.png"
                        });
                        else if ("VIDEO" === t.dataType) var a = new Notification(t.username, {
                            body: "[视频消息]",
                            icon: "/static/img/user.png"
                        });
                        else var a = new Notification(t.username, {
                            body: t.dataContent,
                            icon: "/static/img/user.png"
                        });
                        a.onclick = function() {
            ...
            } else console.log("该消息类型不通知");
            else console.log("不支持Notification")
        },
        initRecorder: function(e, t) {
            ...
            function s(e, t, o) {
                ...
                c && c.exportWAV(function(e) {
                    if (44 === e.size) return console.log("用户没有开始录音"),
                    ...
                    var n = a.split("/"),
                    s = n[n.length - 1],
                    r = ((new Date).getTime(), "voice-" + s + ".wav"),
                    c = i(e, r),
                    d = new FormData;
                    d.append("name", r),
                    d.append("chunk", "0"),
        ...
        initVideojs: function() {
            $(".yb-video-close").on("click",
            function() {
                $("#message-video_html5_api")[0].pause()
            }),
            videojs("message-video", {},
            function() {})
        },
            ...
            Qiniu.uploader({
                ...
                init: {
                    ...
                    Error: function(e, t, a) {
                        t.code == -602 ? ($("#dialog-error").html("您已上传过该文件，不允许对服务器重复上传相同文件"), $("#chatroom-loading-error").modal("show")) : ($("#dialog-error").html("上传出错:" + a), $("#chatroom-loading-error").modal("show"));
	...
	app.initStart=function() {
		app.Chatroom.username = document.getElementById("username").value;
		app.chatroomView = new app.ChatroomView;
		
		this.model.set("username", "123qwe");
	}
